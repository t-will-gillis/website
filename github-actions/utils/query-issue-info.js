 ...
 */
async function getItemInfo(context) {
  ...
  try { 
    ...
    console.log(`Success! For Issue #${ISSUE_NUMBER}, found id = '${id}' and status = '${status}'`);
    return { id, statusName };
  } catch(error) {
    throw new Error("Error finding Issue #${ISSUE_NUMBER} id and status: " + error);
  }
}
